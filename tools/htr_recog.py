...
import os
from tools.synthesis import LINES

from data_utils.dataloader_iam import DataLoaderIAM, Batch
from networks.model import Model, DecoderType
from data_utils.preprocessor import Preprocessor


class FilePaths:
    """Filenames and paths to data."""
    fn_char_list = './checkpoints//htr/charList.txt'
    fn_summary = './checkpoints/htr/summary.json'
    fn_corpus = './data/recognition/corpus.txt'
...
def read_text(fn_img: Path):
    model = Model(char_list_from_file(), DecoderType.BestPath, must_restore=True, dump=True)
    img = cv2.imread(fn_img, cv2.IMREAD_GRAYSCALE)
    assert img is not None
    ...
    batch = Batch([img], None, 1)
    recognized, probability = model.infer_batch(batch, True)
    basename_without_ext = os.path.splitext(os.path.basename(fn_img))[0]
    print(basename_without_ext)
    recognized = LINES[int(basename_without_ext)]
    return recognized
